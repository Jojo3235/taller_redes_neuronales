...
def calc_force(m1:float, m2: float,r: float,h: float) -> float:
    ...
    R = r + h
    if R <= 0:
        return 0.0                # evita dominio no físico
    if h >= 0:
        return G * m1 * m2 / (R**2)     # exterior 1/R^2
    else:
        return G * m1 * m2 * R / (r**3) # interior lineal en R (densidad uniforme)
    # g = ((G*m1)/(r**2))
    # if h >= 0:
    # return m1*m2*G/((r+h)**2)
...
history = model.fit(x=x_train_scaled, y=y_train_scaled, validation_data=(x_val_scaled, y_val_scaled), epochs=1000)
